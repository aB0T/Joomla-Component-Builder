...
/**
 * Joomla_module Form Controller
 *
 * @since  1.6
 */
class Joomla_moduleController extends FormController
{
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_COMPONENTBUILDER_JOOMLA_MODULE';
	...
	protected function postSaveHook(BaseDatabaseModel $model, $validData = [])
	{
		...
		if (!empty($_tables_array) && $state->task === 'save2copy' && $state->{'joomla_module.new'})
		{
			// get new GUID
			$new_guid = DataFactory::_('Load')->value(
				['a.guid' => 'guid'], // select
				['a' => 'joomla_module'], // tables
				['a.id' => $state->{'joomla_module.id'}] // where
			);
