...
class Joomla_powersController extends AdminController
{
	...
	public function pullPowers()
	{
		...
		// set default error message
		$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_PERMISSION_DENIED') . '</h1>';
		$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_YOU_DO_NOT_HAVE_PERMISSION_TO_PULL_JOOMLA_POWERS') . '</p>';
		$status = 'error';
		$success = false;

		if($user->authorise('joomla_power.pull', 'com_componentbuilder'))
		{
			// set success message
			$message = null;
			...
			// set redirect
			$redirect_url = Route::_('index.php?option=com_componentbuilder&view=pull_selection&power=Joomla.Power&target=Joomla Powers', false);
		}
		else
		{
			// set redirect
