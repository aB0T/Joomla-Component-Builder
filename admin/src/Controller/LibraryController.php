...
/**
 * Library Form Controller
 *
 * @since  1.6
 */
class LibraryController extends FormController
{
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_COMPONENTBUILDER_LIBRARY';
	...
	protected function postSaveHook(BaseDatabaseModel $model, $validData = [])
	{
		...
		if (!empty($_tables_array) && $state->task === 'save2copy' && $state->{'library.new'})
		{
			// get new GUID
			$new_guid = DataFactory::_('Load')->value(
				['a.guid' => 'guid'], // select
				['a' => 'library'], // tables
				['a.id' => $state->{'library.id'}] // where
			);
