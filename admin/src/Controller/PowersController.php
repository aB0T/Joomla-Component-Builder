...
class PowersController extends AdminController
{
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Power', $prefix = 'Administrator', $config = ['ignore_request' => true])
	{
	...
	public function pullPowers()
	{
		...
		// set default error message
		$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_PERMISSION_DENIED') . '</h1>';
		$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_YOU_DO_NOT_HAVE_PERMISSION_TO_PULL_POWERS') . '</p>';
		$status = 'error';
		$success = false;

		if($user->authorise('power.pull', 'com_componentbuilder'))
		{
			// set success message
			$message = null;
			...
			// set redirect
			$redirect_url = Route::_('index.php?option=com_componentbuilder&view=pull_selection&power=Power&target=Powers', false);
		}
		else
		{
			// set redirect
