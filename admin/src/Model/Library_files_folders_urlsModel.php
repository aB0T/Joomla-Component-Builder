 ...
 * @since  1.6
 */
class Library_files_folders_urlsModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_componentbuilder/assets/css/library_files_folders_urls.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_componentbuilder/js/library_files_folders_urls.js'
 	];
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'library_files_folders_urls', $prefix = 'Administrator', $config = [])
	{
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('library_files_folders_urls.delete', 'com_componentbuilder.library_files_folders_urls.' . (int) $record->id);
	}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_componentbuilder.edit.library_files_folders_urls.data', []);
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			// Only for strings
			if (UtilitiesStringHelper::check($this->table->library) && !is_numeric($this->table->library))
			{
