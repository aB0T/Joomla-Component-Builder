...
class TemplateModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_componentbuilder/assets/css/template.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_componentbuilder/js/template.js'
 	];
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('core.delete', 'com_componentbuilder.template.' . (int) $record->id);
	}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_componentbuilder.edit.template.data', []);
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('core.create') && !$this->canDo->get('template.batch'))
		{
			return false;
		}
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('core.edit') && !$this->canDo->get('template.batch'))
		{
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				{
					$msg = Text::_('COM_COMPONENTBUILDER_TEMPLATE_SAVE_WARNING');
				}
