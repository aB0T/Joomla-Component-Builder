...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('admin_fields.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('admin_fields.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('admins_fields.publish')->listCheck(true);

				$childBar->unpublish('admins_fields.unpublish')->listCheck(true);

				$childBar->archive('admins_fields.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('admins_fields.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('admins_fields.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('admins_fields.trash')->listCheck(true);
				}
			}
		}
