...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canDo = Actions::get('admin_fields_conditions');
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('admin_fields_conditions.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('admin_fields_conditions.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('admins_fields_conditions.publish')->listCheck(true);

				$childBar->unpublish('admins_fields_conditions.unpublish')->listCheck(true);

				$childBar->archive('admins_fields_conditions.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('admins_fields_conditions.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('admins_fields_conditions.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('admins_fields_conditions.trash')->listCheck(true);
				}
			}
		}
