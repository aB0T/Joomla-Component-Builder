...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('class_extends.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('class_extends.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('class_extendings.publish')->listCheck(true);

				$childBar->unpublish('class_extendings.unpublish')->listCheck(true);

				$childBar->archive('class_extendings.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('class_extendings.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('class_extendings.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('class_extendings.trash')->listCheck(true);
				}
			}
		}
