...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canDo = Actions::get('class_property');
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('class_property.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('class_property.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('class_properties.publish')->listCheck(true);

				$childBar->unpublish('class_properties.unpublish')->listCheck(true);

				$childBar->archive('class_properties.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('class_properties.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('class_properties.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('class_properties.trash')->listCheck(true);
				}
			}
		}
