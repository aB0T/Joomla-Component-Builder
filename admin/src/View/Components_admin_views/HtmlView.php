...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canDo = Actions::get('component_admin_views');
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('component_admin_views.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('component_admin_views.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('components_admin_views.publish')->listCheck(true);

				$childBar->unpublish('components_admin_views.unpublish')->listCheck(true);

				$childBar->archive('components_admin_views.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('components_admin_views.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('components_admin_views.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('components_admin_views.trash')->listCheck(true);
				}
			}
		}
