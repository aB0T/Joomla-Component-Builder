...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canDo = Actions::get('component_config');
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('component_config.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('component_config.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('components_config.publish')->listCheck(true);

				$childBar->unpublish('components_config.unpublish')->listCheck(true);

				$childBar->archive('components_config.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('components_config.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('components_config.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('components_config.trash')->listCheck(true);
				}
			}
		}
