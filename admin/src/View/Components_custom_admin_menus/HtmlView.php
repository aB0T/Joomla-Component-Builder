...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('component_custom_admin_menus.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('component_custom_admin_menus.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('components_custom_admin_menus.publish')->listCheck(true);

				$childBar->unpublish('components_custom_admin_menus.unpublish')->listCheck(true);

				$childBar->archive('components_custom_admin_menus.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('components_custom_admin_menus.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('components_custom_admin_menus.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('components_custom_admin_menus.trash')->listCheck(true);
				}
			}
		}
