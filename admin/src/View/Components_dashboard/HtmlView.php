...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('component_dashboard.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('component_dashboard.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('components_dashboard.publish')->listCheck(true);

				$childBar->unpublish('components_dashboard.unpublish')->listCheck(true);

				$childBar->archive('components_dashboard.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('components_dashboard.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('components_dashboard.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('components_dashboard.trash')->listCheck(true);
				}
			}
		}
