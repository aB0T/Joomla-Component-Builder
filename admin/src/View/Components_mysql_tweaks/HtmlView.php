...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canDo = Actions::get('component_mysql_tweaks');
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('component_mysql_tweaks.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('component_mysql_tweaks.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('components_mysql_tweaks.publish')->listCheck(true);

				$childBar->unpublish('components_mysql_tweaks.unpublish')->listCheck(true);

				$childBar->archive('components_mysql_tweaks.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('components_mysql_tweaks.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('components_mysql_tweaks.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('components_mysql_tweaks.trash')->listCheck(true);
				}
			}
		}
