...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('component_plugins.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('component_plugins.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('components_plugins.publish')->listCheck(true);

				$childBar->unpublish('components_plugins.unpublish')->listCheck(true);

				$childBar->archive('components_plugins.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('components_plugins.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('components_plugins.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('components_plugins.trash')->listCheck(true);
				}
			}
		}
