 ...
 */
namespace VDM\Component\Componentbuilder\Administrator\View\Fieldtypes;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('fieldtype.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('fieldtypes.publish')->listCheck(true);

				$childBar->unpublish('fieldtypes.unpublish')->listCheck(true);

				$childBar->archive('fieldtypes.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('fieldtypes.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('fieldtypes.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('fieldtypes.trash')->listCheck(true);
				}
			}
		}
