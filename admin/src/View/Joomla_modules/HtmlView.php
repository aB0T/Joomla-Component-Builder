 ...
 */
namespace VDM\Component\Componentbuilder\Administrator\View\Joomla_modules;
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canDo = Actions::get('joomla_module');
	...
	protected function addToolbar(): void
	{
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canState)
			{
				...
				{
					$childBar->checkin('joomla_modules.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('joomla_modules.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('joomla_modules.trash')->listCheck(true);
				}
			}
		}
