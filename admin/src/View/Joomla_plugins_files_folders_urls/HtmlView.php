 ...
 */
namespace VDM\Component\Componentbuilder\Administrator\View\Joomla_plugins_files_folders_urls;
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canDo = Actions::get('joomla_plugin_files_folders_urls');
	...
	{
		ToolbarHelper::title(Text::_('COM_COMPONENTBUILDER_JOOMLA_PLUGINS_FILES_FOLDERS_URLS'), 'joomla');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();

		if ($this->canCreate)
		{
			$toolbar->addNew('joomla_plugin_files_folders_urls.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('joomla_plugin_files_folders_urls.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('joomla_plugins_files_folders_urls.publish')->listCheck(true);

				$childBar->unpublish('joomla_plugins_files_folders_urls.unpublish')->listCheck(true);

				$childBar->archive('joomla_plugins_files_folders_urls.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('joomla_plugins_files_folders_urls.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('joomla_plugins_files_folders_urls.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('joomla_plugins_files_folders_urls.trash')->listCheck(true);
				}
			}
		}
