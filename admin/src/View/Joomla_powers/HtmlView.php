...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('joomla_power.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('joomla_power.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('joomla_powers.publish')->listCheck(true);
				$childBar->unpublish('joomla_powers.unpublish')->listCheck(true);
				$childBar->archive('joomla_powers.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('joomla_powers.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('joomla_powers.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('joomla_powers.trash')->listCheck(true);
				}
			}
		}
