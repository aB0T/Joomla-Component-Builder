 ...
 */
namespace VDM\Component\Componentbuilder\Administrator\View\Layouts;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('layout.edit')->listCheck(true);
			}
			...
			if ($this->canState)
			{
				...
				{
					$childBar->checkin('layouts.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('layouts.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
