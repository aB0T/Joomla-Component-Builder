 ...
 */
namespace VDM\Component\Componentbuilder\Administrator\View\Libraries;
...
class HtmlView extends BaseHtmlView
{
	...
	{
		ToolbarHelper::title(Text::_('COM_COMPONENTBUILDER_LIBRARIES'), 'puzzle');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();

		if ($this->canCreate)
		{
			$toolbar->addNew('library.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('library.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('libraries.publish')->listCheck(true);

				$childBar->unpublish('libraries.unpublish')->listCheck(true);

				$childBar->archive('libraries.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('libraries.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('libraries.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('libraries.trash')->listCheck(true);
				}
			}
		}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => Text::_('COM_COMPONENTBUILDER_LIBRARY_NAME_LABEL'),
			'a.target' => Text::_('COM_COMPONENTBUILDER_LIBRARY_TARGET_LABEL'),
			'a.type' => Text::_('COM_COMPONENTBUILDER_LIBRARY_TYPE_LABEL'),
			'a.description' => Text::_('COM_COMPONENTBUILDER_LIBRARY_DESCRIPTION_LABEL'),
