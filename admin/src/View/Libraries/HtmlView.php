...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			ToolbarHelper::addNew('library.add');
		}
		...
		if (ArrayHelper::check($this->items))
		{
			if ($this->canEdit)
			{
				ToolbarHelper::editList('library.edit');
			}

			if ($this->canState)
			{
				ToolbarHelper::publishList('libraries.publish');
				ToolbarHelper::unpublishList('libraries.unpublish');
				ToolbarHelper::archiveList('libraries.archive');

				if ($this->canDo->get('core.admin'))
				{
					ToolbarHelper::checkin('libraries.checkin');
				}
			}

			if ($this->state->get('filter.published') == -2 && ($this->canState && $this->canDelete))
			{
				ToolbarHelper::deleteList('', 'libraries.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
