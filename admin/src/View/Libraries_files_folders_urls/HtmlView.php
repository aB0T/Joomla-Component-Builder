 ...
 */
namespace VDM\Component\Componentbuilder\Administrator\View\Libraries_files_folders_urls;
...
class HtmlView extends BaseHtmlView
{
	...
	{
		ToolbarHelper::title(Text::_('COM_COMPONENTBUILDER_LIBRARIES_FILES_FOLDERS_URLS'), 'joomla');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();

		if ($this->canCreate)
		{
			$toolbar->addNew('library_files_folders_urls.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('library_files_folders_urls.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('libraries_files_folders_urls.publish')->listCheck(true);

				$childBar->unpublish('libraries_files_folders_urls.unpublish')->listCheck(true);

				$childBar->archive('libraries_files_folders_urls.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('libraries_files_folders_urls.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('libraries_files_folders_urls.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('libraries_files_folders_urls.trash')->listCheck(true);
				}
			}
		}
