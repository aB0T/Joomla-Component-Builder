 ...
 */
namespace VDM\Component\Componentbuilder\Administrator\View\Placeholders;
...
class HtmlView extends BaseHtmlView
{
	...
	{
		ToolbarHelper::title(Text::_('COM_COMPONENTBUILDER_PLACEHOLDERS'), 'search');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();

		if ($this->canCreate)
		{
			$toolbar->addNew('placeholder.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('placeholder.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('placeholders.publish')->listCheck(true);

				$childBar->unpublish('placeholders.unpublish')->listCheck(true);

				$childBar->archive('placeholders.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('placeholders.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('placeholders.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('placeholders.trash')->listCheck(true);
				}
			}
		}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.target' => Text::_('COM_COMPONENTBUILDER_PLACEHOLDER_TARGET_LABEL'),
