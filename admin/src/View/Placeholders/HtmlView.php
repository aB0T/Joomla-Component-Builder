...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			ToolbarHelper::addNew('placeholder.add');
		}
		...
		if (ArrayHelper::check($this->items))
		{
			if ($this->canEdit)
			{
				ToolbarHelper::editList('placeholder.edit');
			}

			if ($this->canState)
			{
				ToolbarHelper::publishList('placeholders.publish');
				ToolbarHelper::unpublishList('placeholders.unpublish');
				ToolbarHelper::archiveList('placeholders.archive');

				if ($this->canDo->get('core.admin'))
				{
					ToolbarHelper::checkin('placeholders.checkin');
				}
			}

			if ($this->state->get('filter.published') == -2 && ($this->canState && $this->canDelete))
			{
				ToolbarHelper::deleteList('', 'placeholders.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
