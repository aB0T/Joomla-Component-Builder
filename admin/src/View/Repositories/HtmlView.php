	...
	protected function addToolbar(): void
	{
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canState)
			{
				...
				{
					$childBar->checkin('repositories.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('repositories.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('repositories.trash')->listCheck(true);
				}
			}
		}
