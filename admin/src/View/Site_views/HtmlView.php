 ...
 */
namespace VDM\Component\Componentbuilder\Administrator\View\Site_views;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('site_view.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('site_views.publish')->listCheck(true);

				$childBar->unpublish('site_views.unpublish')->listCheck(true);

				$childBar->archive('site_views.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('site_views.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('site_views.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('site_views.trash')->listCheck(true);
				}
			}
		}
