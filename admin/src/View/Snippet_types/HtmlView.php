 ...
 */
namespace VDM\Component\Componentbuilder\Administrator\View\Snippet_types;
...
class HtmlView extends BaseHtmlView
{
	...
	{
		ToolbarHelper::title(Text::_('COM_COMPONENTBUILDER_SNIPPET_TYPES'), 'joomla');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();

		if ($this->canCreate)
		{
			$toolbar->addNew('snippet_type.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			{
				$childBar->edit('snippet_type.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('snippet_types.publish')->listCheck(true);

				$childBar->unpublish('snippet_types.unpublish')->listCheck(true);

				$childBar->archive('snippet_types.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('snippet_types.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('snippet_types.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('snippet_types.trash')->listCheck(true);
				}
			}
		}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => Text::_('COM_COMPONENTBUILDER_SNIPPET_TYPE_NAME_LABEL'),
			'a.description' => Text::_('COM_COMPONENTBUILDER_SNIPPET_TYPE_DESCRIPTION_LABEL'),
