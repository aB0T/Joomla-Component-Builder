...
$edit = "index.php?option=com_componentbuilder&view=components_routers&task=component_router.edit";
...
<?php foreach ($this->items as $i => $item): ?>
	...
	<tr class="row<?php echo $i % 2; ?>">
		...
		<td class="hidden-phone">
			<?php echo Text::_($item->mode_constructor_before_parent); ?>
		</td>
		<td class="hidden-phone">
			<?php echo Text::_($item->mode_constructor_after_parent); ?>
		</td>
		<td class="hidden-phone">
		...
		<?php if (!$this->isModal && $canDo->get('component_router.edit.state')) : ?>
				<?php if ($item->checked_out) : ?>
					...
					<?php else: ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'components_routers.', false, 'cb'); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php echo Html::_('jgrid.published', $item->published, $i, 'components_routers.', true, 'cb'); ?>
				<?php endif; ?>
