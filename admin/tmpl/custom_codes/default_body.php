...
<?php foreach ($this->items as $i => $item): ?>
	...
	<tr class="row<?php echo $i % 2; ?>">
		...
		<?php if (!$this->isModal && $canDo->get('custom_code.edit.state')): ?>
			<?php
				$iconClass = '';
				if (!$this->saveOrder)
				{
		...
		<?php if (!$this->isModal && $canDo->get('custom_code.edit')): ?>
				...
				<?php else: ?>
					...
					<?php else: ?>
						<?php
							...
							$attribs = 'data-content-select data-content-type="com_componentbuilder.custom_code"'
								. ' data-id="' . $dataId . '"'
		...
		<td class="hidden-phone">
			<?php echo Text::_($item->comment_type); ?>
		</td>
		<td class="hidden-phone">
			<div>				<?php if ($item->target_code == 1): ?>
					<?php echo "Joomla  {$item->joomla_version}"; ?>
				<?php else: ?>
					#<?php echo Text::_('COM_COMPONENTBUILDER_ALL'); ?>
				<?php endif; ?>
			</div>
		</td>
		<td class="center">
		<?php if (!$this->isModal && $canDo->get('custom_code.edit.state')) : ?>
				<?php if ($item->checked_out) : ?>
					<?php if ($canCheckin) : ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'custom_codes.', true, 'cb'); ?>
					<?php else: ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'custom_codes.', false, 'cb'); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php echo Html::_('jgrid.published', $item->published, $i, 'custom_codes.', true, 'cb'); ?>
				<?php endif; ?>
