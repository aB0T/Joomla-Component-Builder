...
<?php foreach ($this->items as $i => $item): ?>
	...
	<tr class="row<?php echo $i % 2; ?>">
		...
		<?php if (!$this->isModal && $canDo->get('joomla_module.edit.state')): ?>
			<?php
				$iconClass = '';
				if (!$this->saveOrder)
				{
		...
		<?php if (!$this->isModal && $canDo->get('joomla_module.edit')): ?>
			...
			<?php else: ?>
				...
				<?php else: ?>
					<?php
						...
						$attribs = 'data-content-select data-content-type="com_componentbuilder.joomla_module"'
							. ' data-id="' . $dataId . '"'
