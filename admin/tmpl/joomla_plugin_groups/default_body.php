...
<?php foreach ($this->items as $i => $item): ?>
	...
	<tr class="row<?php echo $i % 2; ?>">
		...
		<td class="nowrap">
			<div class="name">
				<?php if (!$this->isModal && $canDo->get('core.edit')): ?>
					...
					<?php if ($item->checked_out): ?>
						<?php echo Html::_('jgrid.checkedout', $i, $userChkOut->name, $item->checked_out_time, 'joomla_plugin_groups.', $canCheckin); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php if (!$this->isModal): ?>
					...
					<?php else: ?>
						<?php
							...
							$attribs = 'data-content-select data-content-type="com_componentbuilder.joomla_plugin_group"'
								. ' data-id="' . $dataId . '"'
		...
		<?php if (!$this->isModal && $canDo->get('core.edit.state')) : ?>
				<?php if ($item->checked_out) : ?>
					<?php if ($canCheckin) : ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'joomla_plugin_groups.', true, 'cb'); ?>
					<?php else: ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'joomla_plugin_groups.', false, 'cb'); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php echo Html::_('jgrid.published', $item->published, $i, 'joomla_plugin_groups.', true, 'cb'); ?>
				<?php endif; ?>
