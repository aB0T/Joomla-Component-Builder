...
final class Data implements PluginDataInterface
{
	...
	public function exists($plugin = null): bool
	{
		...
		elseif (isset($this->index[$plugin]))
		{
			return true;
		}
	...
	public function set($plugin): bool
	{
		...
		if (GuidHelper::valid($plugin))
		{
			$query = $this->getQuery($plugin, 'guid');
		}
		else
		{
			$query = $this->getQuery($plugin);
		}
	 ...
	 * get plugin data
	 *
	 * @param   string   $query   The plugin query
	 *
	 * @return  object|null  The plugin data
	 * @since   5.0.4
	 */
	private function getData($query): ?object
	{
		...
		if ($this->db->getNumRows())
		{
			...
			// set the placeholders for this plugin
			$this->setPlaceholders($plugin);

			// set some description for this plugin
			$this->setDescription($plugin);

			// set some readme for this plugin
	 ...
	 * @since  5.0.4
	 */
	private function setReadme(object $plugin): void
	{
	 ...
	 * @since  5.0.4
	 */
	private function setDescription(object $plugin): void
	{
	 ...
	 * @since  5.0.4
	 */
	private function setPlaceholders(object $plugin): void
	{
